...
while ($d = mysqli_fetch_assoc($q)) {
	...
	if ($i==10 and $cadmin_level==1) break; //show only 10 for player
}

$player_rank_th = "th";
if($rank_player % 10 == 1) $player_rank_th = "st";
if($rank_player % 10 == 2) $player_rank_th = "nd";
if($rank_player % 10 == 3) $player_rank_th = "rd";
...
if($prodi!=''){
	...
	while ($d = mysqli_fetch_assoc($q)) {
		...
		if ($i==10 and $admin_level==1) break; //show only 10 for player
	}
...
# ================================================
$s = "SELECT 1 from tb_chal WHERE id_room='$cid_room' and chal_status=1";
...
if(mysqli_num_rows($q)!=1) die("Error @room_var: menghitung total_chal_point");
...
if(mysqli_num_rows($q)!=1) die("Error @room_var: menghitung jumlah_chal_claimed");
$d=mysqli_fetch_assoc($q);
...
if(mysqli_num_rows($q)!=1) die("Error @room_var: menghitung jumlah_chal_unclaim");
$d=mysqli_fetch_assoc($q);
...
if(mysqli_num_rows($q)!=1) die("Error @room_var: menghitung jumlah_chal_unver");
$d=mysqli_fetch_assoc($q);
...
if(mysqli_num_rows($q)!=1) die("Error @room_var: menghitung jumlah_tugas");
$d=mysqli_fetch_assoc($q);
