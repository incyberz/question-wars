...
# GET VARIABLES
# ================================================
if(!isset($_GET['chal_level_filter'])) die(erjx(4)); $chal_level_filter = $_GET['chal_level_filter'];
if(trim($chal_level_filter)=="") die("Error @ajax. SQL Values Data #4 is empty.");

if(!isset($_GET['chal_name_filter'])) die(erjx(5)); $chal_name_filter = $_GET['chal_name_filter'];
// if(trim($chal_name_filter)=="") die("Error @ajax. SQL Values Data #5 is empty.");

if(!isset($_GET['status_beaten_filter'])) die(erjx(6)); $status_beaten_filter = $_GET['status_beaten_filter'];
if(trim($status_beaten_filter)=="") die("Error @ajax. SQL Values Data #6 is empty.");

if(!isset($_GET['order_by_filter'])) die(erjx(7)); $order_by_filter = $_GET['order_by_filter'];
if(trim($order_by_filter)=="") die("Error @ajax. SQL Values Data #7 is empty.");
...
if($chal_level_filter!="all") $sql_level = " a.chal_level = '$chal_level_filter' ";
...
if(mysqli_num_rows($q)==0){
	...
	while ($d=mysqli_fetch_assoc($q)) {
		...
		if($sifat_chal!=''){
			if(strtolower($sifat_chal)=='wajib') $sifat_chal_show = '<span class="badge badge-danger">Wajib</span>';
			if(strtolower($sifat_chal)=='spesial') $sifat_chal_show = '<span class="badge badge-info">Spesial</span>';
		}else{
			$sifat_chal_show = '';
		}
		...
		if($admin_level==1){
			...
			if(mysqli_num_rows($qq)>1) die("Tidak boleh beaten_by ganda");
			...
			if(mysqli_num_rows($qq)==1){
				...
				if($is_claimed==1) $status_beaten_show = "<span class='badge badge-success'>Claimed</span>";
			...
			if($status_beaten_filter=="all"){
				echo "$output";
			}else if($status_beaten_filter=="unverified" and $is_beaten and !$is_approved and !$is_claimed){
				echo "$output";
			}else if($status_beaten_filter=="unclaimed" and $is_approved and !$is_claimed){
				echo "$output";
			}else if($status_beaten_filter=="claimed" and $is_claimed){
				echo "$output";
			}
		...
		if($admin_level==2 or $admin_level==9){
			...
			if($jumlah_unverified>0) $jumlah_unverified_show = "<span class='badge badge-info'>$jumlah_unverified Unverified</span>";
