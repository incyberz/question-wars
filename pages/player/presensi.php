...
if (mysqli_num_rows($q)==0) {
    ...
    while ($d=mysqli_fetch_assoc($q)) {
        ...
        if (mysqli_num_rows($qq)==1) {
            ...
            $a_dapus3 = '';
            if ($bahan_ajar!="") {
                $a_bahan_ajar = "<a href='$bahan_ajar' target='_blank'>$img_bahan_ajar</a>";
            }
            if ($video_materi!="") {
                $a_video_materi = "<a href='$video_materi' target='_blank'>$img_video_materi</a>";
            }
            if ($dapus1!="") {
                $a_dapus1 = "<a href='$dapus1' target='_blank'>$img_dapus1</a>";
            }
            if ($dapus2!="") {
                $a_dapus2 = "<a href='$dapus2' target='_blank'>$img_dapus2</a>";
            }
            if ($dapus3!="") {
                $a_dapus3 = "<a href='$dapus3' target='_blank'>$img_dapus3</a>";
            }

            $kelengkapan_icons = "$a_bahan_ajar $a_video_materi $a_dapus1 $a_dapus2 $a_dapus3";
        }
        if ($kelengkapan_icons!="") {
            $kelengkapan_icons = "<div style='margin: 5px 0 10px 0'>$kelengkapan_icons</div>";
        }
        ...
        if ($zdate_jadwal!="") {
            $jadwal_show="<span class='badge badge-danger' id='date_jadwal__$id_room_subject'>$zdate_jadwal</span>";
        }
        if ($zdate_jadwal!="" and $xjadwal<=$xskg) {
            $jadwal_show="<span class='badge badge-success' id='date_jadwal__$id_room_subject'>$zdate_jadwal</span>";
        }

        $open_show="<span class='badge badge-success' id='date_open__$id_room_subject'>none</span>";
        if ($zdate_open!="") {
            $open_show="<span class='badge badge-danger' id='date_open__$id_room_subject'>$zdate_open</span>";
        }
        if ($zdate_open!="" and $xopen<=$xskg) {
            $open_show="<span class='badge badge-success' id='date_open__$id_room_subject'>$zdate_open</span>";
        }

        $close_show="<span class='badge badge-success' id='date_close__$id_room_subject'>none</span>";
        if ($zdate_close!="") {
            $close_show="<span class='badge badge-warning' id='date_close__$id_room_subject'>$zdate_close</span>";
        }
        if ($zdate_close!="" and $xclose>=$xskg) {
            $close_show="<span class='badge badge-success' id='date_close__$id_room_subject'>$zdate_close</span>";
        }
        ...
        if ($jumlah_soal_in_room_subjects>=$jumlah_soal_minimal) {
            $tipe_badge_jumlah_soal = "success";
        }
        ...
        if (mysqli_num_rows($qq)==1) {
            $st_klp_dosen = "<span class='badge badge-success'>Ready</span>";
        }
        ...
        if ($count_presensi_skg<$count_presensi_player) {
            $count_presensi_skg=$count_presensi_player;
        }
...
<section id="presensi" class="player">
	<div class="container">
		...
		<?php
        if ($status_room==-1) {
            echo '<div class="alert alert-danger">Maaf, sudah tidak bisa Melengkapi Presensi karena Room telah berakhir.</div>';
        }
?>
