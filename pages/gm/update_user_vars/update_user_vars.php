...
$no=0;
$selisih_detik = 0;
$s1 = "SELECT a.nickname, b.kelas,c.prodi 
...
";
$q1 = mysqli_query($cn, $s1) or die(mysqli_error($cn));
while ($d1=mysqli_fetch_assoc($q1)) {
    ...
    # ================================================
    $s = "SELECT last_update from tmp_user_var where id_room=$cid_room and nickname='$d1[nickname]'";
    $q = mysqli_query($cn, $s) or die("Error #room_var1xc Can't get room data.". mysqli_error($cn));
    if (mysqli_num_rows($q)>1) {
        die('duplicate rows');
    } elseif (mysqli_num_rows($q)==1) {
        $d = mysqli_fetch_assoc($q);

        $selisih_detik = strtotime('now') - strtotime($d['last_update']);
        if ($selisih_detik<3600) {
            echo "<h1>continued $selisih_detik $d1[nickname]</h1>";
            continue;
        }
    }
    ...
    while ($d = mysqli_fetch_assoc($q)) {
        $rank_player++;
        if (strtoupper($d['nickname'])==strtoupper($d1['nickname'])) {
            break;
        }
    }
...
//     } //show only 10 for player
    ...
    JOIN tb_room_subject b on a.id_room_subject=b.id_room_subject 
    JOIN tb_room c on b.id_room=c.id_room 
    WHERE a.soal_creator='$d1[nickname]' 
    and c.id_room='$cid_room'
    ";
    ...
    # ================================================
    $id_user_var = $cid_room."_$d1[nickname]";
    ...
    echo "<h1>$no. $selisih_detik User Vars $d1[nickname] Update Success. Please wait system will auto refresh!</h1>";
}
